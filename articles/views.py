...
def article_create(request):
    ...
    if request.method == "POST":
        ...
        if article_post_form.is_valid():
            ...
            # 如果你进行过删除数据表的操作，可能会找不到id=1的用户
            # 此时请重新创建用户，并传入此用户的id
            # 因为我登录后台的用户名是charolim，发布文章也是这个用户，通过
            # 查询数据库blodweb中的quth_user表中的数据，找到charolim这个用户的id是4，因为将下面的
            # id设置成4
            new_article.author = User.objects.get(id=8)  # 8/10
            # 新增的代码
...
def article_delete(request, id):
    ...
    return redirect("articles:article_list") # 完成删除后返回文章列表
